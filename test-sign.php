...
if ($last === null) {
    ...
    if ($last->side == 'BUY' && $last->price > 0) {
        ...
        $price = $_GET['buy'];
        $btcAmt = number_format((getCoinAmount('USDT') / $price) * 0.9985, 5);
        list($hi, $lo, $cur) = lastTickInfo();
        if ($price > $cur) {
            $output->error = "Requested buy price $price above current $cur";
        } else {
            $output->debug = "symbol=BTCUSDT&side=BUY&type=LIMIT&timeInForce=GTC&quantity=$btcAmt&price=$price";
            $output->order = req('order', "symbol=BTCUSDT&side=BUY&type=LIMIT&timeInForce=GTC&quantity=$btcAmt&price=$price", true, 'POST');
        }
    }
}
